...
# ~ class FermVaultApp(App):
    ...
    # ~ def push_data_update(self, **kwargs):
        ...
        # ~ if hasattr(self, 'settings_manager'):
            ...
            # ~ # -- FG (With added \n for padding & Logic Analysis) --
            # ~ # Based on ui_manager_base.py analysis:
            # ~ # fg_value_var holds the number (e.g. 1.010)
            # ~ # fg_status_var holds the status text (e.g. "Stable", "Checking...", or "")
            # ~ fg_val = self.settings_manager.get("fg_value_var", "-.---")
            # ~ fg_msg = self.settings_manager.get("fg_status_var", "--")
            
            # ~ # If msg is empty, default to dashes
            # ~ if not fg_msg: fg_msg = "--"
                
            # ~ self.fg_full_text = f"FG: {fg_val}\n\n{fg_msg}"
            
            # ~ # -- FG COLOR LOGIC --
            # ~ # Match Tkinter behavior: Green if "Stable", else White
            # ~ if fg_msg == "Stable":
                # ~ self.fg_text_color = [0.2, 0.8, 0.2, 1] # Green
            # ~ else:
                # ~ self.fg_text_color = [1, 1, 1, 1] # White
